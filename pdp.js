...
function pdpGetTotalFGForRef(codart_wip, code_client) {
  ...
  if (!nom) {
    // Pas de nomenclature : sommer stock du WIP brut + stock du FG final (si différents)
    const stkWip = _stk(codart_wip) + _enc(codart_wip);
    const stkFg  = (code_client && code_client !== codart_wip)
      ? _stk(code_client) + _enc(code_client)
      : 0;
    return stkWip + stkFg;
  }
  ...
  // Le ratio de l'étape i = combien d'unités de i pour faire 1 unité de i+1
  // On divise le stock de chaque étape par SON propre ratio (pas un cumul)
  return nom.etapes.reduce((s, e) => {
    const ratio = (e.ratio && e.ratio > 1) ? e.ratio : 1;
    const stk = _stk(e.codart) + _enc(e.codart);
    return s + (ratio > 1 ? Math.floor(stk / ratio) : stk);
  }, 0);
}
...
function pdpAutoShortName(r) {
  ...
  const lenSeg = segs.find(s => /^L\d{2,3}$/i.test(s));
  ...
  for (let i = 0; i < segs.length - 1; i++) {
    if (/^[A-Z]{1,3}$/i.test(segs[i]) && /^\d/.test(segs[i+1])) {
      return segs[i].toUpperCase() + segs[i+1] + lenSuffix;
    }
  }
  // Variante : un seul segment du type "K101", "RS1208" ou "AE2" déjà collé
  // (1 à 4 chiffres : AE1/AE2/AE3 n'ont qu'un seul chiffre, contrairement à K09/K135)
  const collé = segs.find(s => /^[A-Z]{1,3}\d{1,4}(\.\d+)?$/i.test(s));
  if (collé) return collé.toUpperCase() + lenSuffix;

  // 3. Segment qui contient à la fois des chiffres ET des lettres, même si l'ordre ne
  //    colle pas au motif "collé" ci-dessus (ex: "80.04SG" = chiffres D'ABORD, puis
  //    lettres). Ce segment est déjà suffisamment spécifique en lui-même — il ne faut
  //    PAS aller chercher un numéro dans le libellé à la place, ça donnerait un nom
  //    bien trop générique (ex: "0.5" partagé par DT Light #0.5, DT Ill XRO #0.5,
  //    NTI #0.5... toutes des références DIFFÉRENTES).
  const numSeg = segs.find(s => /\d/.test(s) && /[A-Z]/i.test(s));
  if (numSeg) return numSeg.toUpperCase() + lenSuffix;

  // 4. Repli : chercher dans le libellé un nombre après "Size"/"#" (ex: "Size 1", "#1.2").
  //    Seulement si AUCUN segment du codart_wip ne contient de chiffre — càd que le code
  ...
  // 5. Repli final : premier segment du codart_wip s'il est court, sinon tronqué
...
// Les segments génériques de procédé/matière (JTR, JTO, JNT, XRO, couleurs...) sont
// ignorés : ils apparaissent quasi tels quels sur des dizaines de références RTD
// totalement différentes (ex: 80.04SG_JTR, 90.06SG_JTR, AE2_JTR partagent tous
// "_JTR" sans aucun rapport entre eux) — les compter aurait fait remonter des OF
// sans rapport dès qu'un nom court devenait ambigu.
const _PDP_GENERIC_SEGMENTS = new Set([
  'JTR','JTO','JNT','XRO','MDR','JAR','SG','IND','INDC','C',
  'BLANC','ROSE','BLEU','JAUNE','NOIR','ROUGE','VRAC',
]);
...
function pdpGetOFsForRef(codart_wip, shortName) {
  ...
  return ofs.filter(o => {
    ...
    //     (ex: OF "TENON K09_L27_JNTXRO_1 LOT4") : sens toujours fiable.
    ...
    //    isolé, pas juste une sous-chaîne, pour éviter que W1 matche W10/W11, ou que
    //    "K09" matche "K09L27" par erreur. Si le nom court est partagé par plusieurs
    //    codart_wip différents (ex: "AE2" = AE2_ILLUSION_X-RO ET AE2_JTR, deux familles
    //    distinctes), on ne le désactive pas complètement — sinon un OF légitimement
    //    identifiable ("AE2 ill XRO") ne matcherait plus rien — mais on départage par
    //    le nombre de segments du codart_wip retrouvés (même abrégés) dans le produit
    //    de l'OF, et on n'accepte QUE si cette référence l'emporte strictement sur
    //    toutes les autres candidates partageant le même nom court. Un OF réellement
    //    ambigu (ex: juste "AE2", sans indice distinctif) n'est alors rapproché
    //    d'AUCUNE des deux : mieux vaut ne rien rapprocher que rapprocher à tort.
    if (targetShort) {
