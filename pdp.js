    ...
    + (ofsModal.length > 0
      ? '<div style="margin-bottom:0">'
        ...
        + ofsModal.map(o => {
            ...
            return '<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:var(--bg);border-radius:var(--radius)">'
              + '<span style="font-size:11px;font-weight:700;font-family:monospace;color:var(--accent)">'+o.id+'</span>'
              + '<span style="flex:1;font-size:12px;color:var(--text)">'+o.produit+'</span>'
              + '<span style="font-size:12px;font-weight:600;color:var(--text)">'+o.qty.toLocaleString('fr')+' u.</span>'
              + '<span style="font-size:10px;padding:2px 8px;border-radius:20px;background:#FFF4E6;color:#92400E">'+(sLbls[o.stage]||o.stage)+'</span>'
              + '</div>';
          }).join('')
        + '</div></div>'
      : '')
    ...
    loadXLSXForPdp(() => {
      try {
        ...
        if (typeof showToast === 'function') {
          showToast('✓ ' + newCorrespondances.length + ' références, ' + newNomenclatures.length + ' nomenclatures importées');
        }
