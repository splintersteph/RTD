...
function renderPdpPage() {
  ...
  if (!pdpSelectedClient) {
    ...
    sortedRefs.forEach(r => {
      ...
      // Nom personnalisé ou court
      const displayName = r._shortName || r.libelle_fg;

      mainContent +=
        '<div data-pdp-action="goto-stock" data-pdp-arg="' + r.codart_wip + '" style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:12px 14px;transition:box-shadow .15s;display:flex;flex-direction:column;gap:8px;cursor:pointer"'
        ...
        +'</div>'
        +'<div style="font-size:10px;color:var(--text-faint);font-family:monospace;margin-top:1px">'+r.code_client+' \u2192 '+r.codart_wip+'</div>'
        +'</div>'
        ...
        // Encadré Stock / Commandes / Rupture
        +'<div style="display:flex;gap:6px;margin-top:4px;padding-top:8px;border-top:1px solid var(--border)">'
        +'<div style="flex:1;background:var(--bg);border-radius:6px;padding:6px 8px;text-align:center">'
        +'<div style="font-size:9px;color:var(--text-faint);text-transform:uppercase;letter-spacing:.03em">Stock</div>'
        +'<div style="font-size:13px;font-weight:700;color:'+col.text+'">'+totalFG.toLocaleString('fr')+'</div>'
        +'</div>'
        +'<div style="flex:1;background:var(--bg);border-radius:6px;padding:6px 8px;text-align:center">'
        +'<div style="font-size:9px;color:var(--text-faint);text-transform:uppercase;letter-spacing:.03em">Commandé</div>'
        +'<div style="font-size:13px;font-weight:700;color:#A32D2D">'+cde.toLocaleString('fr')+'</div>'
        +'</div>'
        +'<div style="flex:1.4;background:'+(rupture?'#FCEBEB':'#EAF3DE')+';border-radius:6px;padding:6px 8px;text-align:center">'
        +'<div style="font-size:9px;color:'+(rupture?'#A32D2D':'#27500A')+';text-transform:uppercase;letter-spacing:.03em">Rupture</div>'
        +'<div style="font-size:12px;font-weight:700;color:'+(rupture?'#A32D2D':'#27500A')+'">'+(rupture?pdpFmtDate(rupture.date):'\u2014')+'</div>'
        +(rupture?'<div style="font-size:8px;color:#A32D2D;margin-top:1px">manque '+rupture.manque.toLocaleString('fr')+'</div>':'')
        +'</div>'
        +'</div>'

        // Bouton réordonner (drag handle)
        +'<div style="display:flex;justify-content:flex-end;margin-top:2px">'
        +'<button data-pdp-action="moveup" data-pdp-arg="'+r.code_client+'" style="background:none;border:none;cursor:pointer;padding:2px 5px;color:var(--text-faint);font-size:11px" title="Monter"><i class="ti ti-arrow-up"></i></button>'
        +'<button data-pdp-action="movedown" data-pdp-arg="'+r.code_client+'" style="background:none;border:none;cursor:pointer;padding:2px 5px;color:var(--text-faint);font-size:11px" title="Descendre"><i class="ti ti-arrow-down"></i></button>'
        +'</div>'
