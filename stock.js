...
function renderStockPage() {
  ...
  if (stockSelectedRef && hasStock) {
    const nomForRef = (typeof nomenclatures !== 'undefined')
      ? nomenclatures.find(n => n.etapes && n.etapes.some(e => e.codart === stockSelectedRef))
      : null;

    let dispo, enCours;
    if (nomForRef) {
      // IMPORTANT : diviser chaque niveau par son ratio CUMULÉ jusqu'au FG (pas la
      // somme brute des quantités de chaque étape) — un tenon et une boîte finie ne
      // valent pas "1 unité" chacun, il faut le même calcul que pdpShowDetail/
      // pdpGetTotalFGForRef (pdp.js), sous peine d'afficher un "en cours" gonflé
      // (ex: 15033 au lieu de ~1533, en confondant tenons bruts et unités FG).
      ...
      nomForRef.etapes.forEach((e, i) => {
        ...
        const enc = stockGetEnCoursProd(e.codart);
      ...
      dispo   = stockGetDisponible(stockSelectedRef);
      enCours = stockGetEnCoursProd(stockSelectedRef);
    }
