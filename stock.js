...
function renderStockPage() {
  ...
  if (stockSelectedRef && hasStock) {
    const dispo    = stockGetDisponible(stockSelectedRef);
    const enCours  = stockGetEnCoursProd(stockSelectedRef);
    const total    = dispo + enCours;
    const cmds     = stockGetCommandes(stockSelectedRef);
  ...
  const refRows = filteredRefs.map(ref => {
    ...
    const cmds    = stockGetCommandes(ref);
    const totalCde= cmds.reduce((s,c)=>s+c.qteRest,0);
