...
function stockGetDisponible(codart) {
  const EMPL_OK  = ['BSRTD1','BFRI01'];
  const EMPL_NOK = ['R&D','KATNC','ECHANT','SARNC','ECH'];
  return (pcData.stock||[])
    .filter(r => {
      const art  = String(r.CODART||'').trim();
      const empl = String(r.EMPLAC||'').trim();
      return art === codart && EMPL_OK.some(e=>empl===e) && !EMPL_NOK.some(e=>empl===e);
    })
...
function renderStockPage() {
  ...
  if (stockSelectedRef && hasStock) {
    ...
    let dispo, enCours;
    if (nomForRef) {
      dispo   = nomForRef.etapes.reduce((s,e) => s + stockGetDisponible(e.codart), 0);
      enCours = nomForRef.etapes.reduce((s,e) => s + stockGetEnCoursProd(e.codart), 0);
    } else {
