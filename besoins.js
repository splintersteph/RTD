...
function renderBesoinsPage() {
  ...
  corrs.forEach(r => {
    ...
    let totalFG = 0;
    if (nom) {
      totalFG = nom.etapes.reduce((s, e, i) => {
        let cumRatio = 1;
        ...
        const fgEquiv = cumRatio > 1 ? Math.floor((stk + enc) / cumRatio) : (stk + enc);
        return s + fgEquiv;
      }, 0);
    } else {
      const stk = (typeof stockGetDisponible === 'function') ? stockGetDisponible(r.codart_wip) : 0;
      const enc = (typeof stockGetEnCoursProd === 'function') ? stockGetEnCoursProd(r.codart_wip) : 0;
      totalFG = stk + enc;
    }
