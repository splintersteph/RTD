...
function renderConsoPage() {
  ...
  const nbCritique = rows.filter(x => x.couverture !== null && x.couverture < 1).length;
  const nbSurveiller = rows.filter(x => x.couverture !== null && x.couverture >= 1 && x.couverture < 3).length;
  ...
  let html = '<div style="flex:1;overflow-y:auto;padding:0;display:flex;flex-direction:column">'
    ...
    + '<input id="conso-search" placeholder="Filtrer (référence, client)…" value="'+consoSearchFilter+'" oninput="consoSearch(this.value)" style="padding:6px 10px;border:1px solid var(--border-med);border-radius:var(--radius);background:var(--bg);color:var(--text);font-size:12px;font-family:var(--font);outline:none;width:240px">'
  ...
  if (nbSansDonnee === rows.length && rows.length) {
    html += '<div style="margin-bottom:14px;padding:12px 16px;background:var(--bg);border:1px dashed var(--border-med);border-radius:var(--radius);font-size:12px;color:var(--text-faint);text-align:center">'
      + '<i class="ti ti-info-circle" style="font-size:18px;display:block;margin-bottom:6px"></i>'
      + 'Aucune référence ne trouve de livraison correspondante dans les données de ventes chargées.</div>';
  }

  // Regroupement par client : on garde l'ordre du tri déjà calculé (rows est déjà
  // trié selon consoSortField) et on regroupe SANS le perturber — le groupe du
  // premier client rencontré (donc contenant la ligne la plus prioritaire au sens
  // du tri courant) apparaît en premier, cohérent avec le tri choisi.
  const groupsOrder = [];
  const groupsMap = {};
  rows.forEach(x => {
    const cl = x.r.client;
    if (!groupsMap[cl]) { groupsMap[cl] = []; groupsOrder.push(cl); }
    groupsMap[cl].push(x);
  });

  groupsOrder.forEach(cl => {
    const groupRows = groupsMap[cl];
    const nbCritiqueGroupe = groupRows.filter(x => x.couverture !== null && x.couverture < 1).length;

    html += '<div style="margin-bottom:16px;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden">'
      + '<div style="padding:10px 16px;background:var(--bg);border-bottom:1px solid var(--border);display:flex;align-items:center;gap:10px">'
      + '<span style="font-size:13px;font-weight:700">'+cl+'</span>'
      + '<span style="font-size:11px;color:var(--text-faint)">'+groupRows.length+' réf.</span>'
      + (nbCritiqueGroupe ? '<span style="font-size:11px;font-weight:600;padding:2px 9px;border-radius:20px;background:#FCEBEB;color:#A32D2D;margin-left:auto">'+nbCritiqueGroupe+' &lt; 1 mois</span>' : '')
      + '</div>'
