...
function cdeShowOrderDetail(numCmd) {
  ...
  const rowsHtml = lines.map(r => {
    const stock = (typeof pdpGetTotalFGForRef === 'function')
    ...
    const stockBadge = stock === null
      ...
      : '<span style="font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;background:'+(ok?'#EAF3DE':'#FCEBEB')+';color:'+(ok?'#27500A':'#A32D2D')+'"><i class="ti '+(ok?'ti-check':'ti-x')+'" style="vertical-align:-1px;margin-right:3px;font-size:10px"></i>'+stock.toLocaleString('fr')+'</span>';
  ...
  document.getElementById('modal').innerHTML = `
    ...
    <div class="modal-body">
      ...
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden">
        <div style="overflow-x:auto"><table class="cat-table" style="min-width:600px">
